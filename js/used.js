window.addEventListener('load', () => {
    ...
    // Fetch used ads
    fetch('https://auto-mart-api.herokuapp.com/api/v1/car/state/used', {
        method: 'get',
        headers: {
            'Authorization': token
        },
        ...
        const ads = data.used_cars;
        if (ads) {
            document.getElementById('used-count').innerHTML = `${ads.length} Ads found`;
            const usedAdsCard = document.getElementById('prod-card');
            usedAdsCard.innerHTML = `${ads.map(usedCarAds).join('')}`;
        } else {
            usedAdsCard.innerHTML = data.error;
        }
